...
if (isset($_POST['addStudent'])) {
    ...
    $stmt = $pdo->prepare("INSERT INTO studentlogin (username, email, password) VALUES (?, ?, ?)");
...
if (isset($_POST['updateStudent'])) {
    ...
    $stmt = $pdo->prepare("UPDATE studentlogin SET username = ?, email = ?, password = ? WHERE id = ?");
...
if (isset($_GET['delete_id'])) {
    $id = $_GET['delete_id'];
    $stmt = $pdo->prepare("DELETE FROM studentlogin WHERE id = ?");
    $stmt->execute([$id]);
    header("Location: student-login.php");
}

// Fetch all student logins
$stmt = $pdo->query("SELECT * FROM studentlogin");
...
<head>
    ...
    <title>Institute Admin Panel - Student Web Login</title>
            ...
            <tbody>
                <?php foreach ($students as $student): ?>
                    <tr>
                        ...
                        <td>
                        <button onclick="openEditModal('<?= $student['id'] ?>', '<?= $student['username'] ?>', '<?= $student['email'] ?>')">Edit</button>| 
                            <a href="student-login.php?delete_id=<?= $student['id'] ?>" onclick="return confirm('Are you sure you want to delete this student?')">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        <button class="add-teacher-btn" onclick="openAddModal()">Add Student</button>
