...
if (isset($_POST['addTeacher'])) {
    ...
    $stmt = $pdo->prepare("INSERT INTO teacherlogin1 (`full_name`, email, phone, password) VALUES (?, ?, ?, ?)");
...
if (isset($_POST['updateTeacher'])) {
    ...
    $stmt = $pdo->prepare("UPDATE teacherlogin1 SET `full_name` = ?, email = ?, phone = ?, password = ? WHERE id = ?");
...
if (isset($_GET['delete_id'])) {
    $id = $_GET['delete_id'];
    $stmt = $pdo->prepare("DELETE FROM teacherlogin1 WHERE id = ?");
    $stmt->execute([$id]);
    header("Location: teacher.php");
}

// Fetch all teachers
$stmt = $pdo->query("SELECT * FROM teacherlogin1");
...
<head>
    ...
    <title>Institute Admin Panel - Teachers</title>
            ...
            <tbody>
                <?php foreach ($teachers as $teacher): ?>
                    <tr>
                        ...
                        <td>
                            <button onclick="openEditModal('<?= $teacher['id'] ?>', '<?= $teacher['full_name'] ?>', '<?= $teacher['email'] ?>', '<?= $teacher['phone'] ?>', '<?= $teacher['password'] ?>')">Edit</button>
                            <a href="teacher.php?delete_id=<?= $teacher['id'] ?>" onclick="return confirm('Are you sure you want to delete this teacher?')">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        <button class="add-teacher-btn" onclick="openAddModal()">Add Teacher</button>

       <!-- Add Teacher Modal -->
       <div id="addModal" class="modal">
            <div class="modal-content">
                <span class="close" onclick="closeAddModal()">&times;</span>
                <div class="modal-header">Add New Teacher</div>
