...
if (isset($_POST['addTimetable'])) {
    ...
    $stmt = $pdo->prepare("INSERT INTO timetable1 (subject, grade, teacher_name, time, class_fee) VALUES (?, ?, ?, ?, ?)");
    $stmt->execute([$subject, $grade, $teacher_name, $time, $class_fee]);
...
if (isset($_POST['updateTimetable'])) {
    ...
    $stmt = $pdo->prepare("UPDATE timetable1 SET subject = ?, grade = ?, teacher_name = ?, time = ?, class_fee = ? WHERE id = ?");
    $stmt->execute([$subject, $grade, $teacher_name, $time, $class_fee, $id]);
    header("Location: timetable.php");
}
...
if (isset($_GET['delete_id'])) {
    $id = $_GET['delete_id'];
    $stmt = $pdo->prepare("DELETE FROM timetable1 WHERE id = ?");
    $stmt->execute([$id]);
    header("Location: timetable.php");
}

// Fetch all timetable entries
$stmt = $pdo->query("SELECT * FROM timetable1");
...
<head>
    ...
    <title>Institute Admin Panel - Timetable</title>
...
<body>
    <div class="sidebar">
        ...
        <button onclick="window.location.href='student_login.php'">Student Web Login</button>
    ...
    <div class="content">
            ...
            <tbody>
                <?php foreach ($timetables as $timetable): ?>
                    <tr>
                        ...
                        <td><?= $timetable['class_fee'] ?></td>
                        <td><?= $timetable['created_at'] ?></td>
                        <td>
                        <button onclick="openEditModal('<?= $timetable['id'] ?>', '<?= $timetable['subject'] ?>', '<?= $timetable['grade'] ?>', '<?= $timetable['teacher_name'] ?>', '<?= $timetable['time'] ?>', '<?= $timetable['class_fee'] ?>')">Edit</button>| 
                            <a href="timetable.php?delete_id=<?= $timetable['id'] ?>" onclick="return confirm('Are you sure you want to delete this timetable entry?')">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        <button class="add-teacher-btn" onclick="openAddModal()">Add New Subject</button>
       ...
       <div id="addModal" class="modal">
            <div class="modal-content">
                ...
                <div class="modal-header">Add New Subject</div>
                <form action="timetable.php" method="POST">
                <input type="text" name="subject" placeholder="Subject" required>
                <input type="text" name="grade" placeholder="Grade" required>
                <input type="text" name="teacher_name" placeholder="Teacher Name" required>
                <input type="time" name="time" required>
                <input type="number" name="class_fee" placeholder="Class Fee" step="0.01" required>
                    <button type="submit" name="addTimetable">Add Student</button>
        ...
        <div class="modal-content">
            ...
            <div class="modal-header">Edit Subjects</div>
            <form action="timetable.php" method="POST">
            <input type="hidden" name="id" id="editId">
                ...
                <input type="number" name="class_fee" id="editClass_fee" required>
                <button type="submit" name="updateTimetable">Update Student</button>
    ...
    <script>
        ...
        function openEditModal(id, subject, grade, teacher_name, time, class_fee) {
    document.getElementById('editId').value = id;
    document.getElementById('editSubject').value = subject;
    document.getElementById('editGrade').value = grade;
    document.getElementById('editTeacherName').value = teacher_name;
    document.getElementById('editTime').value = time;
    document.getElementById('editClass_fee').value = class_fee;
