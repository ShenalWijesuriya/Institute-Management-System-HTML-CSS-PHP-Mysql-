...
if (isset($_POST['addCourse'])) {
    ...
    $stmt = $pdo->prepare("INSERT INTO courses (title, time, price, date) VALUES (?, ?, ?, ?)");
    $stmt->execute([$title, $time, $price, $date]);
...
if (isset($_POST['updateCourse'])) {
    ...
    $stmt = $pdo->prepare("UPDATE courses SET title = ?, time = ?, price = ?, date = ? WHERE id = ?");
    $stmt->execute([$title, $time, $price, $date, $id]);
    header("Location: courses.php");
}
...
<head>
    ...
    <title>Institute Admin Panel - Courses</title>
...
<body>
            ...
            <tbody>
                <?php foreach ($courses as $course): ?>
                    <tr>
                        ...
                        <td><?= $course['date'] ?></td>
                        <td>
                            <button onclick="openEditModal(<?= $course['id'] ?>, '<?= addslashes($course['title']) ?>', '<?= $course['time'] ?>', <?= $course['price'] ?>, '<?= $course['date'] ?>')">Edit</button>
                            <a href="courses.php?delete_id=<?= $course['id'] ?>" onclick="return confirm('Are you sure?')">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        <button class="add-teacher-btn" onclick="openAddModal()">Add Course</button>

        <!-- Add Course Modal -->
        <div id="addModal" class="modal">
            <div class="modal-content">
                <span class="close" onclick="closeAddModal()">&times;</span>
                <div class="modal-header">Add New Course</div>
                <form action="courses.php" method="POST">
                    <input type="text" name="title" placeholder="Course Title" required>
                    <input type="text" name="time" placeholder="Course Time" required>
                    <input type="number" name="price" placeholder="Price" required>
                    <input type="text" name="date" placeholder="Course Date" required>
        ...
        <div id="editModal" class="modal">
            <div class="modal-content">
                ...
                <form action="courses.php" method="POST">
                    ...
                    <input type="text" name="time" id="editTime" required>
                    <input type="number" name="price" id="editPrice" required>
                    <input type="text" name="date" id="editDate" required>
    ...
    <script>
        // Updated JavaScript functions without image handling
        function openEditModal(id, title, time, price, date) {
